...
if (!isset($pos) || ($pos === false)) $pos = array_search('--help', $t_args);
...
if (!isset($pos) || ($pos === false)) $pos = array_search('--output-file', $t_args);
if (isset($pos) && ($pos !== false) && isset($t_args[$pos + 1])) {
    $target = $t_args[$pos + 1];
    array_splice($t_args, $pos, 2);           // remove the 2 args and reorder
} else $target = '';
...
if (($x = getenv('YAKPRO_PO_CONFIG_FILENAME')) !== false) $config_file_namepart = $x;
...
foreach ($t_where as $dummy => $where) if (check_config_file($where)) {
