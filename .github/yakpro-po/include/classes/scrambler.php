...
class Scrambler
{
    ...
    private $scramble_length_min = null; // minlength of scrambled names
    private $scramble_length_max = null; // maxlength of scrambled names
    private $t_ignore = []; // array where keys are names to ignore.
    private $t_ignore_prefix = []; // array where keys are prefix of names to ignore.
    ...
    private $t_reserved_method_names = ['__construct', '__destruct', '__call', '__callstatic', '__get', '__set', '__isset', '__unset', '__sleep', '__wakeup', '__tostring', '__invoke', '__set_state', '__clone', '__debuginfo'];
    ...
    private function str_scramble($s) // scramble the string according parameters
    {
        ...
        for ($i = 0, $l = $this->scramble_length - 1; $i < $l; ++$i) {
            $s .= str_replace("=", "", base64_encode(convert_uuencode(str_rot13(gzdeflate($this->t_chars[base_convert(substr($this->r, 2 * $i, 2), 16, 10) % ($this->l2 + 1)])))));
        }
        return $s;
    }
